...
def compute_psd_all_components(samples, fs):
    psd_results = {}
    for i, comp in enumerate(['u', 'v', 'w']):
        time_series = samples[:, i]  # velocity component time series
        freqs, psd = compute_psd(time_series, fs)
        psd_results[comp] = (freqs, psd)
    return psd_results